...
def _build_preview_assets(video_path: str, results: Dict, max_frames: int = 6, max_crops: int = 9):
    preview_frames: List[np.ndarray] = []
    preview_crops: List[np.ndarray] = []

    frame_detections = results.get("frame_detections", {})
    frame_numbers = sorted(frame_detections.keys())

    if frame_numbers and Path(video_path).exists():
        cap = cv2.VideoCapture(video_path)
        if cap.isOpened():
            for frame_number in frame_numbers[:max_frames]:
                cap.set(cv2.CAP_PROP_POS_FRAMES, int(frame_number))
                ret, frame = cap.read()
                if not ret:
                    continue

                for det in frame_detections.get(frame_number, []):
    ...
    for crop in results.get("cropped_images", [])[:max_crops]:
        crop_path = crop.get("path")
        if crop_path and Path(crop_path).exists():
            img = cv2.imread(crop_path)
            if img is not None:
                preview_crops.append(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))

    return preview_frames, preview_crops


def _render_preview(preview_slot) -> None:
    if st.session_state.preview_frames:
        preview_slot.markdown("**Detected Frames**")
        for idx, frame in enumerate(st.session_state.preview_frames, start=1):
            preview_slot.image(frame, caption=f"Frame Preview {idx}", width="stretch")

    if st.session_state.preview_crops:
        preview_slot.markdown("**Detection Crops**")
        cols = preview_slot.columns(3)
        for idx, crop in enumerate(st.session_state.preview_crops):
            with cols[idx % 3]:
                st.image(crop, caption=f"Crop {idx + 1}", width="stretch")

    if not st.session_state.preview_frames and not st.session_state.preview_crops:
        preview_slot.info("Video preview will appear here")
...
if 'preview_crops' not in st.session_state:
    st.session_state.preview_crops = []
...
with tab1:
    ...
    with col2:
        ...
        _render_preview(preview_placeholder)
        ...
        if uploaded_file is not None:
            ...
            if st.button("🔍 Analyze Video", width="stretch", type="primary"):
                ...
                try:
                    ...
                    # Build preview assets before cleaning up temp file
                    preview_frames, preview_crops = _build_preview_assets(video_path, results)
                    st.session_state.preview_frames = preview_frames
                    st.session_state.preview_crops = preview_crops
                    _render_preview(preview_placeholder)
                    ...
                    # Reset verified results on new analysis
                    st.session_state.verified_results = None
...
with tab3:
    ...
    if st.session_state.get('verified_results'):
        ...
        with col4:
            denominator = results['total_frames'] // results['frame_skip'] if results['frame_skip'] else 0
            detection_rate = (results['detected_frames'] / denominator * 100) if denominator else 0
